...
def get_api_config(
    ...
    Args:
        ...
        query: The user's query (used for complexity-based model selection)
...
def get_llm_response(
    ...
    Args:
        ...
        base_url: OpenAI compatible base URL. If None, uses DEEPSEEK_BASE_URL from .env
        model: Model name to use. If None, auto-selects based on task complexity
        knowledge_base: Optional KnowledgeBase instance for RAG. If None, uses _get_kb()
    ...
    # RAG Retrieval - use passed knowledge_base or fall back to global _get_kb()
    kb = knowledge_base if knowledge_base is not None else _get_kb()
    kb_context = ""
    if kb:
...
"""
    ...
    # Use the kb_context retrieved earlier for the RAG context

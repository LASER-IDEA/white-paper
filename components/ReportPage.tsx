...
const ReportPage: React.FC<PageProps> = ({ data, pageNumber }) => {
  ...
  return (
    <div className="w-[210mm] h-[297mm] bg-white shadow-2xl mx-auto my-8 p-[15mm] flex flex-col relative page-break overflow-hidden">
        ...
        <div className="flex flex-col items-end">
            <div className="flex items-baseline space-x-2">
             ...
             <div className={`flex items-center ${trendColor} font-medium mt-1`}>
                ...
                <span>{Math.abs(data.trend * 100).toFixed(1)}% vs prev period</span>
             </div>
        </div>
      </div>

      {/* 2. Main Chart Area (Middle 45%) */}
      <div className="h-[45%] w-full bg-slate-50 rounded-xl p-4 mb-6 border border-slate-100 relative">
      ...
      <div className="h-[15%] grid grid-cols-3 gap-6 mb-8">
        {data.keyMetrics.map((metric, idx) => (
          <div key={idx} className="bg-white rounded-lg border border-slate-100 p-4 flex flex-col justify-center shadow-sm hover:shadow-md transition-shadow">
      ...
      <div className="flex-1 flex flex-col space-y-4">
         <div className="flex items-start space-x-3">
             <div className="w-1 h-full min-h-[20px] bg-slate-300 rounded-full mt-1"></div>
             <div>
                 <span className="block text-xs font-bold text-slate-400 uppercase mb-1">Definition</span>
                 <p className="text-sm text-slate-700 leading-relaxed">{data.definition}</p>
             </div>
         </div>
         <div className="flex items-start space-x-3">
             <div className="w-1 h-full min-h-[20px] bg-tech-blue-500 rounded-full mt-1"></div>
             <div>
                 <span className="block text-xs font-bold text-tech-blue-500 uppercase mb-1">Data Insight</span>
                 <p className="text-sm text-slate-700 leading-relaxed">{data.insight}</p>
             </div>
         </div>
         <div className="flex items-start space-x-3">
             <div className="w-1 h-full min-h-[20px] bg-low-altitude-green-500 rounded-full mt-1"></div>
             <div>
                 <span className="block text-xs font-bold text-low-altitude-green-500 uppercase mb-1">Strategic Suggestion</span>
